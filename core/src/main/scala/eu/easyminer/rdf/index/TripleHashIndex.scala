...
object TripleHashIndex {
  ...
  def apply(triples: Traversable[CompressedTriple]): TripleHashIndex = {
  ...
  def apply(triples: Traversable[Triple])(implicit map: Triple => CompressedTriple): TripleHashIndex = apply(triples.view.map(map))
