...
import eu.easyminer.rdf.utils.extensions.IteratorExtension._
import eu.easyminer.rdf.utils.{Debugger, IncrementalInt, TypedKeyMap}
...
trait RuleExpansion extends AtomCounting {
  ...
  def minHeadCoverage: Double

  val tripleIndex: TripleHashIndex
  val isWithInstances: Boolean
  val onlyObjectInstances: Boolean
  val withDuplicitPredicates: Boolean
  val maxRuleLength: Int
  val atomMatcher: AtomPatternMatcher[Atom]
  val freshAtomMatcher: AtomPatternMatcher[FreshAtom]

  def isValidPredicate(predicate: Int): Boolean

  implicit class PimpedRule(rule: ExtendedRule) {

    private class Projections extends mutable.Map[Atom, IncrementalInt] {
      private val x = collection.mutable.HashMap.empty[Atom, IncrementalInt]

      def +=(kv: (Atom, IncrementalInt)): Projections.this.type = {
        if (matchAtom(kv._1)) {
          x += kv
        }
        this
      ...
      def -=(key: Atom): Projections.this.type = {
        x -= key
        this
      }

      def get(key: Atom): Option[IncrementalInt] = x.get(key)

      def iterator: Iterator[(Atom, IncrementalInt)] = x.iterator
    }

    private class TripleProjections extends mutable.Set[Atom] {
      private val x = collection.mutable.HashSet.empty[Atom]

      def +=(v: Atom): TripleProjections.this.type = {
        if (matchAtom(v)) {
          x += v
        }
        this
      ...
      def -=(elem: Atom): TripleProjections.this.type = {
        x -= elem
        this
      }

      def contains(elem: Atom): Boolean = x.contains(elem)

      def iterator: Iterator[Atom] = x.iterator
    }
    ...
    /**
      ...
      * @return boolean
      */
    private def matchFreshAtom(freshAtom: FreshAtom) = patternAtoms.isEmpty || patternAtoms.exists(x => (x._2.isEmpty && !x._1.exact) || x._2.exists(y => freshAtomMatcher.matchPattern(freshAtom, y)))
    ...
    /**
      ...
      * @return boolean
      */
    private def matchAtom(atom: Atom) = patternAtoms.isEmpty || patternAtoms.exists(x => (x._2.isEmpty && !x._1.exact) || x._2.exists(y => atomMatcher.matchPattern(atom, y)))
    ...
    /**
      ...
      */
    def expand: Iterator[ExtendedRule] = if (rule.patterns.nonEmpty && patternAtoms.forall { case (rp, ap) => rp.exact && ap.isEmpty }) {
      //if all paterns must be exact and are completely matched then we will not expand this rule
      Iterator.empty
    } else {
      implicit val projections: Projections = new Projections
      ...
      debugger.debug("Rule expansion: " + rule, rule.headSize + 1) { ad =>
        ...
        howLong("Rule expansion - count projections", true) {
          rule.headTriples.iterator.zipWithIndex.takeWhile { x =>
            ...
            val selectedAtoms = howLong("Rule expansion - do projection", true)(countProjections(bodySet, possibleFreshAtoms, countableFreshAtoms, specifyHeadVariableMapWithAtom(_subject, _object)))
    ...
    private def countProjections(atoms: Set[Atom], possibleFreshAtoms: List[FreshAtom], countableFreshAtoms: List[FreshAtom], variableMap: VariableMap)
                                (implicit projections: TripleProjections = new TripleProjections): TripleProjections = {
    ...
    private def countFreshAtom(atom: FreshAtom, variableMap: VariableMap)
      ...
      (variableMap.getOrElse(atom.subject, atom.subject), variableMap.getOrElse(atom.`object`, atom.`object`)) match {
        ...
        case (sv: Atom.Variable, Atom.Constant(oc)) =>
          ...
          for ((predicate, subjects) <- tripleIndex.objects.get(oc).iterator.flatMap(_.predicates)) {
            ...
            if (isWithInstances && !onlyObjectInstances && !isCounted(atom, predicate, true)) {
              for (subject <- subjects.keysIterator) projections += Atom(Atom.Constant(subject), predicate, atom.`object`)
            }
        ...
        case (Atom.Constant(sc), ov: Atom.Variable) =>
          ...
          for ((predicate, objects) <- tripleIndex.subjects.get(sc).iterator.flatMap(_.predicates)) {
            if (isWithInstances && !isCounted(atom, predicate, true)) {
              for (_object <- objects.keysIterator) projections += Atom(atom.subject, predicate, Atom.Constant(_object))
            }
            if (!isCounted(atom, predicate, false)) {
        ...
        case (Atom.Constant(sc), Atom.Constant(oc)) =>
          ...
          for (predicate <- tripleIndex.subjects.get(sc).iterator.flatMap(_.objects.get(oc).iterator.flatMap(_.keysIterator)) if !isCounted(atom, predicate, false)) {
    ...
    private def expandWithAtom(atom: Atom, support: Int): ExtendedRule = {
      val newPatterns = patternAtoms.iterator.filter(x => x._2.forall(atomMatcher.matchPattern(atom, _))).map(_._1).toList
          ...
          rule match {
            case rule: DanglingRule => rule.variables match {
              case ExtendedRule.OneDangling(originalDangling, others) =>
                ...
                rule.copy(body = atom +: rule.body)(measures, newPatterns, ExtendedRule.OneDangling(dangling, originalDangling :: others), dangling, rule.headTriples)
              case ExtendedRule.TwoDanglings(dangling1, dangling2, others) =>
                ...
                rule.copy(body = atom +: rule.body)(measures, newPatterns, ExtendedRule.TwoDanglings(dangling, secondDangling, pastDangling :: others), dangling, rule.headTriples)
            }
            case rule: ClosedRule =>
              //(c, a) | (a, b) (a, b) => ClosedRule -> OneDangling(c)
              DanglingRule(atom +: rule.body, rule.head)(measures, newPatterns, ExtendedRule.OneDangling(dangling, rule.variables), dangling, rule.headTriples)
          }
        } else {
          rule match {
            case rule: ClosedRule =>
              //(a, b) | (a, b) (a, b) => ClosedRule -> ClosedRule
              rule.copy(atom +: rule.body)(measures, newPatterns, rule.variables, rule.maxVariable, rule.headTriples)
            case rule: DanglingRule =>
              //(c, a) | (c, a) (a, b) (a, b) => OneDangling(c) -> ClosedRule
              //(c, b) |(a, c) (a, b) => TwoDanglings(c, b) -> ClosedRule
              ClosedRule(atom +: rule.body, rule.head)(measures, newPatterns, rule.variables.danglings ::: rule.variables.others, rule.maxVariable, rule.headTriples)
          }
        }
        case (_: Atom.Variable, _: Atom.Constant) | (_: Atom.Constant, _: Atom.Variable) => rule match {
          ...
          case rule: DanglingRule => rule.variables match {
            case ExtendedRule.OneDangling(dangling, others) =>
              ...
              ClosedRule(atom +: rule.body, rule.head)(measures, newPatterns, dangling :: others, dangling, rule.headTriples)
            case ExtendedRule.TwoDanglings(dangling1, dangling2, others) =>
              ...
              DanglingRule(atom +: rule.body, rule.head)(measures, newPatterns, ExtendedRule.OneDangling(dangling, pastDangling :: others), rule.maxVariable, rule.headTriples)
          }
        }
