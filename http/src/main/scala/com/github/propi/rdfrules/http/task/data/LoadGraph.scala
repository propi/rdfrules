...
/**
  ...
  */
class LoadGraph(graphName: Option[TripleItem.Uri], path: Option[String], url: Option[URL], format: Option[Option[RdfSource]]) extends Task[Task.NoInput.type, Dataset] {
  ...
  def execute(input: Task.NoInput.type): Dataset = format match {
    case Some(Some(x)) =>
      ...
      (path, url) match {
        ...
        case (Some(path), _) => graphName.map(x => Graph(x, path)).getOrElse(Graph(path)).toDataset
    ...
    case Some(None) => (path, url) match {
      ...
      case (Some(path), _) => graphName.map(x => Graph.fromCache(x, path)).getOrElse(Graph.fromCache(path)).toDataset
      case _ => throw ValidationException("NoSource", "No path or url is specified.")
    }
    case None => path match {
      case Some(path) => graphName.map(x => Graph(x, path)).getOrElse(Graph(path)).toDataset
