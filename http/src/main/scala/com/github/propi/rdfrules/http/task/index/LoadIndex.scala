...
class LoadIndex(path: String, partially: Boolean)(implicit debugger: Debugger) extends Task[Task.NoInput.type, IndexPart] {
  val companion: TaskDefinition = LoadIndex

  def execute(input: Task.NoInput.type): IndexPart = IndexPart.fromCache(Workspace.path(path), partially)
}
