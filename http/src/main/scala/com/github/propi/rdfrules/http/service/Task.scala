      ...
      post {
        extractUri { uri =>
          entity(as[Debugger => Pipeline[Source[JsValue, NotUsed]]]) { pipeline =>
            ...
            actorSystem.actorOf(actor.Task.props(id, started, pipeline), "task-" + id.toString)
            complete(StatusCodes.Accepted, List(Location(uri.withPath(uri.path / id.toString))), actor.Task.Response.InProgress(id, started, Nil))
          }
        }
      }
    } ~ path(JavaUUID) { id =>
