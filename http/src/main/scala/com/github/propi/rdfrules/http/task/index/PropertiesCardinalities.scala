...
import com.github.propi.rdfrules.index.{IndexPart, PropertyCardinalities, TripleItemIndex}

class PropertiesCardinalities(filter: Set[TripleItem.Uri]) extends Task[IndexPart, Seq[PropertyCardinalities.Resolved]] {
  val companion: TaskDefinition = PropertiesCardinalities

  def execute(input: IndexPart): Seq[PropertyCardinalities.Resolved] = {
