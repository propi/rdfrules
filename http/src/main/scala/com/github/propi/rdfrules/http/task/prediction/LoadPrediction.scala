...
class LoadPrediction(path: String, format: PredictionSource)(implicit debugger: Debugger) extends Task[IndexPart, PredictedTriples] {
  val companion: TaskDefinition = LoadPrediction

  def execute(input: IndexPart): PredictedTriples = PredictedTriples(input, Workspace.path(path))(Compression.fromPath(path) match {
