...
/**
  ...
  */
class PredictTriples(onlyFunctionalProperties: Boolean, predictionType: PredictionType) extends Task[Ruleset, Dataset] {
  val companion: TaskDefinition = PredictTriples

  def execute(input: Ruleset): Dataset = {
    val predictionResult = input.predictedTriples(predictionType)
