...
/**
  ...
  */
class Predict(testSet: Option[String], mergeTestAndTrainForPrediction: Boolean, onlyTestCoveredPredictions: Boolean, predictedResults: Set[PredictedResult], injectiveMapping: Boolean, headVariablePreMapping: HeadVariablePreMapping)(implicit debugger: Debugger) extends Task[Ruleset, PredictedTriples] {
  ...
  def execute(input: Ruleset): PredictedTriples = {
    ...
    input.predict(testDataset, mergeTestAndTrainForPrediction, onlyTestCoveredPredictions, predictedResults, injectiveMapping, headVariablePreMapping)
  }
