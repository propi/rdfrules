...
class ExportRules(fromOperation: Operation) extends Operation {
  ...
  val properties: Constants[Property] = Constants(
    ...
    new Select("format", "Rules format", Constants("txt" -> "Text (unparsable)", "json" -> "JSON (as model - parsable)"), description = "The output format is automatically detected from the file extension (.txt or .json|.rules). But, you can specify the format explicitly. The 'text' format is human readable, but it can not be parsed for other use in RDFRules, e.g. for completion another dataset. If you need to use the ruleset for other purposes, use the 'json' format or the 'cache' process.")
  )
