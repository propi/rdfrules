...
class ComputePcaConfidence(fromOperation: Operation) extends Operation {
  ...
  val properties: Constants[Property] = Constants(
    new OptionalText[Double]("min", "Min PCA confidence", "0.5", "A minimal PCA (Partial Completeness Assumption) cofidence threshold. This operation counts the PCA confidence for all rules and filter them by this minimal threshold. The value range is between 0.01 and 1 included. Default value is set to 0.5.", RegExp("1(\\.0+)?|0\\.0[1-9]\\d*|0\\.[1-9]\\d*", true))
  )
