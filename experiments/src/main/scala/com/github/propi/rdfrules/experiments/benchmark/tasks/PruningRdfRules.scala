...
class PruningRdfRules(val name: String) extends Task[Ruleset, Ruleset, (Ruleset, Ruleset), Seq[Metric]] with TaskPreProcessor[Ruleset, Ruleset] with TaskPostProcessor[(Ruleset, Ruleset), Seq[Metric]] {
  ...
  protected def postProcess(result: (Ruleset, Ruleset)): Seq[Metric] = {
    ...
    List(
      ...
      Metric.Number("coveredTriples", rules.predict(predictedResults = Set(PredictedResult.Positive)).distinctPredictions.triples.size),
      Metric.Number("prunedCoveredTriples", pruned.predict(predictedResults =Set(PredictedResult.Positive)).distinctPredictions.triples.size)
    )
  }
