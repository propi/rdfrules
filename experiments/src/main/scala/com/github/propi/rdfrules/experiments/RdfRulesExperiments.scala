      ...
      Debugger() { implicit debugger =>
        if (cli.hasOption("rungraphs")) {
          ...
          if (cli.hasOption("runconfidence")) {
            val rules = index.mineRules(Amie().setParallelism(numberOfThreads).addConstraint(RuleConstraint.ConstantsAtPosition(ConstantsPosition.Object)).addThreshold(Threshold.MinHeadCoverage(0.01)), RuleConsumer(TopKRuleConsumer(100000))).cache
          ...
          if (cli.hasOption("runclusters")) {
            val rules = index.mineRules(Amie().setParallelism(numberOfThreads).addThreshold(Threshold.MinHeadCoverage(0.01)), RuleConsumer(TopKRuleConsumer(10000))).cache
          ...
          if (cli.hasOption("runpruning")) {
            ...
            for (topK <- topKs) {
              val rules = index.mineRules(Amie().setParallelism(numberOfThreads).addThreshold(Threshold.MinHeadCoverage(0.01)), RuleConsumer(TopKRuleConsumer(topK))).computeConfidence[Measure.CwaConfidence](0.1).cache
