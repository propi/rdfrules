...
object TripleSerialization {
  ...
  implicit val tripleDeserializer: Deserializer[Triple] = (v: Array[Byte]) => {
    ...
    val s = Deserializer.deserialize[TripleItem.Uri](bais).get
    val p = Deserializer.deserialize[TripleItem.Uri](bais).get
    val o = Deserializer.deserialize[TripleItem](bais).get
    Triple(s, p, o)
  }
