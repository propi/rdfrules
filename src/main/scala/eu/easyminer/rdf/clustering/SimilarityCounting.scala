...
object AtomsSimilarityCounting extends SimilarityCounting {
  ...
  private def itemsSimilarity(p1: Int, p2: Int)(item1: Atom.Item, item2: Atom.Item) = (item1, item2) match {
    ...
    case (x: Atom.Variable, y: Atom.Constant) => if (p1 == p2) 0.5 else 0.0
    case (x: Atom.Constant, y: Atom.Variable) => if (p1 == p2) 0.5 else 0.0
...
trait MeasuresSimilarityCounting extends SimilarityCounting {
  ...
  protected def relativeNumbersSimilarity(v1: Double, v2: Double) = 1 - math.abs(v1 - v2)

  protected def absoluteNumbersSimilarity(v1: Int, v2: Int) = {
...
object SupportSimilarityCounting extends MeasuresSimilarityCounting {
  ...
  def apply(rule1: Rule, rule2: Rule): Double = relativeNumbersSimilarity(rule1.measures(Measure.HeadCoverage).asInstanceOf[Measure.HeadCoverage].value, rule2.measures(Measure.HeadCoverage).asInstanceOf[Measure.HeadCoverage].value)
