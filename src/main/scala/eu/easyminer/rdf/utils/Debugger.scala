...
import eu.easyminer.rdf.utils.Debugger.Messages.{CloseAction, Debug, NewAction}
...
object Debugger {
  ...
  private class ActorDebugger(debugger: ActorRef) extends Debugger {
    ...
    def debug[T](name: String, num: Int = 0)(f: ActionDebugger => T): T = {
      ...
      debugger ! NewAction(name, num)
      try {
        f(ad)
      } finally {
        debugger ! CloseAction(name)
      }
    }
  }
  ...
  private class DebuggerActor extends Actor {
    ...
    def receive: Receive = {
      case NewAction(name, num) =>
        val action = new Action(name, num)
        actions += (name -> action)
        dump(action, "started")
      case Debug(name, msg) => actions.get(name).foreach { action =>
        action.++
        dump(action, msg)
      }
      case CloseAction(name) => actions.get(name).foreach { action =>
        dump(action, "ended")
        actions -= name
      }
    }
