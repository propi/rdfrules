      ...
      Debugger() { implicit debugger =>
        ...
        if (cli.hasOption("runconfidences")) {
          ...
          Functor(DefaultConfidence(Measure.CwaConfidence)).foreach { implicit defaultConfidence =>
            ...
            Once executeTask new PredictionTask[Seq[Metric]]("RDFRules prediction task, CWA, Joint", test, NoisyOrScorer(), TopRules(100)) with ModesPredictionTaskPostprocessor withInput confRuleset andFinallyProcessResultWith BasicPrinter()
          }
          Functor(DefaultConfidence(Measure.PcaConfidence)).foreach { implicit defaultConfidence =>
            val confRuleset = computeConfidence("", ruleset)
            Once executeTask new PredictionTask[Seq[Metric]]("RDFRules prediction task, PCA, Joint", test, NoisyOrScorer(), TopRules(100)) with ModesPredictionTaskPostprocessor withInput confRuleset andFinallyProcessResultWith BasicPrinter()
          }
          Functor(DefaultConfidence(Measure.QpcaConfidence)).foreach { implicit defaultConfidence =>
            Once executeTask new PredictionTask[Seq[Metric]]("RDFRules prediction task, QPCA, Joint", test, NoisyOrScorer(), TopRules(100)) with ModesPredictionTaskPostprocessor withInput exactConfRuleset andFinallyProcessResultWith BasicPrinter()
          }
        }
        if (cli.hasOption("runmodes")) {
          implicit val defaultConfidence: DefaultConfidence = DefaultConfidence(Measure.QpcaConfidence)
          Once executeTask new PredictionTask[Seq[Metric]]("RDFRules prediction without modes", test, NoisyOrScorer(), TopRules(100)) with PredictionTaskPostprocessor withInput exactConfRuleset andFinallyProcessResultWith BasicPrinter()
        }
